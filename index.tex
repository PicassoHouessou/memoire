...
L'objectif est donc de proposer un essai de solution pour aider et faciliter la communication au sein de l'ENEAM. La communication regroupe toutes informations utiles que l'école peut mettre à disposition des étudiants, que les professeurs peuvent échanger avec leurs apprenants, de même que les apprenants peuvent s'échanger entre eux. Je me propose de répondre à cette problématique par la mise en place d'une solution complète de messagerie électronique au sein de mon école.
...
password_query = SELECT email as user, password FROM virtual_users WHERE email='%u' WHERE state=TRUE
\end{exempleConsole}
